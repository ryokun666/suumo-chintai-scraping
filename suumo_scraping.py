import datetime
from input_gs import write_to_spreadsheet
import requests
...
for page in range(1, max_page+1):
    ...
    for child in mother:
        ...
        data_home = []
        # 取得日付
        data_home.append(dt_now.strftime('%Y年%m月%d日 %H:%M:%S'))
        # カテゴリ
        ...
        for room in rooms.find_all(class_='js-cassette_link'):
            ...
            for id_, grandchild in enumerate(room.find_all('td')):
                ...
                # 家賃と管理費
                    ...
                    data_room.append(grandchild.find(
                        class_='cassetteitem_other-emphasis ui-text--bold').text)
                    data_room.append(grandchild.find(
                        class_='cassetteitem_price cassetteitem_price--administration').text)
                # 敷金と礼金
                elif id_ == 4:
                    data_room.append(grandchild.find(
                        class_='cassetteitem_price cassetteitem_price--deposit').text)
                    data_room.append(grandchild.find(
                        class_='cassetteitem_price cassetteitem_price--gratuity').text)
                # 間取りと面積
                elif id_ == 5:
                    data_room.append(grandchild.find(
                        class_='cassetteitem_madori').text)
                    data_room.append(grandchild.find(
                        class_='cassetteitem_menseki').text)
                # url
                elif id_ == 8:
                    get_url = grandchild.find(
                        class_='js-cassette_link_href cassetteitem_other-linktext').get('href')
...
# ここにデータをスプレッドシートに書き込む関数を呼び出す
write_to_spreadsheet(data_samples)
